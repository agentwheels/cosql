...
- `UPDATE` / `DELETE` with a WHERE clause → run a `cosql query` with
...
```sh
cosql propose <db> --sql "UPDATE users SET active=false WHERE id=7" \
...
```sh
cosql propose <db> -f ./migration.sql --note "adds idx_users_email; EXPLAIN estimates 120k rows scanned once"
```
...
```sh
awk -f to-sql.awk data.csv | cosql propose <db> --note "bulk import of N rows"
```
...
next: run `sudo cosql apply a1b2c3d4` in a terminal you control.
inspect: `cosql proposal show a1b2c3d4`
```
...
You do NOT run `sudo cosql apply` yourself. The CLI refuses when
...
```sh
cosql proposal show a1b2c3d4
cosql proposal show a1b2c3d4 --json    # when you need to parse it
```
...
- `rejected` — the user said no (or you ran `cosql proposal reject`).
...
- **Production database?** If the `notes` field in `cosql list`
...
```sh
cosql proposal list                     # all non-expired proposals, newest first
cosql proposal list --status pending    # filter by status
cosql proposal list --json              # for parsing
cosql proposal show <id>                # full details including EXPLAIN
cosql proposal show <id> --json
cosql proposal reject <id>              # mark a pending proposal as rejected
```

Every `apply` attempt — success or failure — is appended to
`~/.local/share/cosql/audit.log`. Read it when you need history:

```sh
cosql query <db> --sql "..."  # NOT this; audit.log is a plain-text file
```
